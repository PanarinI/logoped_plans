...
async def main():
    # Инициализируем асинхронный клиент OpenAI с ключом API и тайм-аутом в 60 секунд
    #client = openai.AsyncOpenAI(
    #    api_key=os.getenv("API_KEY_openai"),  # Получаем ключ API из переменных окружения
    ...
    params = {
        ...
        "наличие_ДЗ": True,  # Наличие домашнего задания (True или False)
    ...
    На основе этих параметров составь подробный конспект занятия, который должен включать:
    ...
    4. **Встроенные упражнения с достаточным количеством материала:** Приведи конкретные упражнения с подробными примерами.
    ...
    6. **Подбор материалов из свободных источников (если разрешен web search = True):** Если разрешен веб-поиск, найди релевантные доступные материалы и интегрируй их в ход занятия.
    7. **Домашнее задание (если предусмотрено):** Если параметр 'наличие_ДЗ' указывает на наличие домашнего задания, предложи конкретное домашнее задание, соответствующее теме занятия.
    """
    ...
    if params['разрешен_web_search']:
        tools.append({"type": "web_search_preview",
                    ...
                    )  # Добавляем инструмент веб-поиска, если разрешено
    ...
    response = client.responses.create(
        ...
        tool_choice={"type": "web_search_preview"},  # Явно указываем инструмент
        max_output_tokens=1000  # Максимальное количество токенов в ответе
    )
