...
def generate_lesson_plan_interface(
    ...
    3. **Ход занятия** – основная часть плана. Логически последовательные этапы с конкретными упражнениями и их примерами
    4. **Домашнее задание** (только если требуется)
    5. **Рекомендации по особым условиям** (только если требуются)
    
    В плане используй минимум общих фраз, максимум конкретики
    ...
    if разрешен_file_search:
        file_search_section = """
  Правила:
- Подходящие примеры упражнений и материалы подбирай СТРОГО из файловой базы
