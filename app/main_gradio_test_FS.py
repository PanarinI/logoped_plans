...
def generate_lesson_plan_interface(
    ...
    3. **Ход занятия** – основная часть плана. Логически последовательные этапы, насыщенный конкретными упражнениями и их примерами
    4. **Домашнее задание** (только если требуется)
    5. **Рекомендации по особым условиям** (только если требуются)
    
    В ответе приведи только план, используй минимум общих фраз, максимум конкретики
    ...
    if разрешен_file_search:
        file_search_section = """
  Правила:
- Составляй план строго на основе упражнений из загруженных источников
- Адаптируй при необходимости упражнения в соответствиями с параметрами занятия и характеристиками ребенка.
- Если подходящих упражнений в источниках нет - сообщи об этом
