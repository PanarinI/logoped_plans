...
def generate_lesson_plan_interface(
    ...
    На основе указанных параметров занятия составь план занятия, который должен включать:
    1. **Тема занятия, цель и задачи** (не более 3-х, кратко)
    2. **Необходимый инвентарь**
    3. **Ход занятия** – основная часть плана. Логически последовательные этапы с конкретными упражнениями и примерами
    4. **Домашнее задание** (только если параметр "наличие домашнего задания" = True)
    5. **Рекомендации по особым условиям** (только если Особые условия = True)
    
    В конспекте используй минимум общих фраз, максимум конкретики
    ...
    if разрешен_file_search:
        file_search_section = """
      Правила:
    - Отвечай строго на основе загруженных файлов
    - Подходящие упражнения и материалы в первую очередь подбирай из файловой базы
    - Если в файловой базе нет подходящих материалов, сообщи об этом в ответе
    - В ответе - минимум общих фраз, максимум конкретики
    """

    # Собираем полный промпт
    full_prompt = base_prompt + file_search_section
