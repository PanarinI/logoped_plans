    ...
    with gr.Row():
        ...
        with gr.Column(elem_classes=["right-col"], scale=2):
            ...
            with gr.Accordion("💬 Помогите нам стать лучше", open=False):
                gr.Markdown(
                    """
                    Спасибо, что попробовали! Как вам?<br>
                    В этом проекте главное - ваши наблюдения и замечания о том, что получается, а что нет.<br>
                    Они позволяют ассистенту становится умнее и полезнее.
                    """
                )
                ...
                rating = gr.Radio(choices=["1", "2", "3", "4", "5"], label="Как вам?",
                                  info="1 — совсем не то, 5 — полезно!", visible=True)  # Можно включить позже
